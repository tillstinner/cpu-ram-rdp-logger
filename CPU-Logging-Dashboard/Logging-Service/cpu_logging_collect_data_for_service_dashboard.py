...
def collect_data_xlsx(filename, should_comtinue, interval=2, laufzeit=0, callback=None):
    ...
    try:
        while should_comtinue():
            ...
            # Log error inside Excel for visibility
            ws_data.append([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "ERROR", str(e)])
            wb.save(filename)
